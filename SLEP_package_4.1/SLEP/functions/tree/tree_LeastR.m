...
%  如果x的每个元素都是一个树上的叶子节点，并且这些叶子节点的权重都一样，我们提供一种非常有效的输入方法就是[-1,-1,lambda]
%
...
%  如果特征按照顺序排列好，左侧树上的节点特征总是少于右侧节点树的，opts.ind(1,:)第一行和opts.ind(2,:)第二行分别表示真实的起始点和终止indices。
%  这就是说 x( opts.ind(1,j):opts.ind(2,j) )表示x_{G_j}，这种情况下，opts.ind(1:2,:)第一行和第二行所有的数目都应该在1,n之间
%  If the features are not well ordered, please use the input opts.G for
%  specifying the index so that  如果特征没有很好按照顺序排列，那么需要使用Opts.G特别指出组的indices。
...
%  这种情况下，opts.G的index是在1-n之间，而opts.ind(1:2,:)在1-length(opts.G)之间
%
