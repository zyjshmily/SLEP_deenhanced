...
%  这里不同的是，使用opts.ind_t表示每个group（x_Gj）的indices
%
...
%    此时，opts.ind表示不同任务的起始indices，比如说第一个任务的data就是A(（opts.ind(1)+1）:(opts.ind(2)))
%
%% Input parameters:
%
...
% compute AT y
for i=1:k% 从第i类开始
    ind_i=(ind(i)+1):ind(i+1);     % indices for the i-th group得到第i类indices
